...
export default function CurriculumPage() {
  ...
  const openEditLesson = (l: Lesson) => {
    ...
    setLType(l.lesson_type);
    setLDuration(String(l.duration_minutes || ""));
    setLessonOpen(true);
  };
  ...
  const handleSaveModule = async () => {
    ...
    try {
      if (editModule) {
        await adminQuery("update_course_module", {
          id: editModule.id,
          title: mTitle.trim(),
        ...
        await adminQuery("create_course_module", {
  ...
  const handleDeleteModule = async (id: string) => {
    ...
    try {
      await adminQuery("delete_course_module", { id });
  ...
  const handleSaveLesson = async () => {
    ...
    try {
      if (editLesson) {
        await adminQuery("update_lesson", {
          id: editLesson.id,
          title: lTitle.trim(),
          content: lContent.trim() || null,
          lesson_type: lType,
          duration_minutes: Number(lDuration) || 0,
        });
        toast.success("Lesson updated");
      } else {
        await adminQuery("create_lesson", {
  ...
  const handleDeleteLesson = async (id: string) => {
    ...
    try {
      await adminQuery("delete_lesson", { id });
      toast.success("Deleted");
      loadModules();
    } catch (e: any) {
      toast.error(e.message);
  ...
  const totalDuration = modules.reduce(
    (s, m) => s + (m.lessons ?? []).reduce((ss, l) => ss + (l.duration_minutes || 0), 0),
    0,
  );
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
          <BookOpen className="h-6 w-6" /> Curriculum
        </h1>
                  ...
                  <Collapsible open={expandedModules.has(m.id)} onOpenChange={() => toggleModule(m.id)}>
                    <CardHeader className="py-3 px-4">
                      <div className="flex items-center justify-between gap-2">
                        <CollapsibleTrigger className="flex items-center gap-2 hover:text-primary transition-colors flex-1 text-left truncate">
                          {expandedModules.has(m.id) ? (
                            <ChevronDown className="h-4 w-4 shrink-0" />
                          ) : (
                            <ChevronRight className="h-4 w-4 shrink-0" />
                          )}
                          <span className="text-xs font-mono text-muted-foreground">M{mi + 1}</span>
                    ...
                    <CollapsibleContent>
                      <CardContent className="pt-0 pb-3 px-4">
                        {!m.lessons || m.lessons.length === 0 ? (
                          <p className="text-sm text-muted-foreground ml-8">No lessons in this module</p>
                        ) : (
      ...
      >
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>Content</Label>
              <Textarea value={lContent} onChange={(e) => setLContent(e.target.value)} rows={4} />
            </div>
