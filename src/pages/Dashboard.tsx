...
export default function Dashboard() {
  ...
  // Debug: Log current organization selection
  useEffect(() => {
    if (profile) {
  ...
  // Fetch user's organizations for viewing (even when org is selected)
  const { data: userOrganizationsList } = useQuery({
    queryKey: ["user_organizations_list", profile?.id],
    queryFn: async () => {
      if (!profile?.id) return [];
      const { data, error } = await supabase
        .from("organization_members")
    ...
    enabled: !!profile && (isSuperadmin || (needsOrgSelection && !!organizationId)),
  });
  ...
  const { data: orgName, refetch: refetchOrgName } = useQuery({
  ...
  // Quick Context Indicator Component
  const QuickContextIndicator = () => (
    <div className="fixed bottom-4 right-4 z-50">
      <div className="bg-black/80 backdrop-blur-md rounded-full px-4 py-2 text-xs border border-white/20 shadow-lg">
        <div className="flex items-center gap-2">
          ...
          {organizationId && !isStudent && !isParent && !isSuperadmin && (
            <>
          ...
          {isSuperadmin && (
            <>
              ...
              <span className="font-medium text-purple-400">Super Admin</span>
            </>
          )}
        </div>
      </div>
  ...
  // Organization Selector Widget Component
  const OrganizationSelector = () => {
    if (!userOrganizationsList || userOrganizationsList.length === 0) return null;
    ...
    return (
      <motion.div
        ...
        className="mb-6 p-4 rounded-xl bg-gradient-to-r from-white/5 to-white/2 border border-white/10 backdrop-blur-sm"
      >
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <p className="text-lg font-semibold text-gradient">{orgName || "No Organization Selected"}</p>
                {organizationId && <CheckCircle className="w-4 h-4 text-green-500" />}
              </div>
            </div>
          </div>

          {userOrganizationsList.length > 1 && (
            <MagneticButton
              onClick={() => setShowOrgSelector(!showOrgSelector)}
              className="text-xs flex items-center gap-1"
            >
              Switch Organization
              <ChevronDown className={`w-3 h-3 transition-transform ${showOrgSelector ? "rotate-180" : ""}`} />
            </MagneticButton>
          )}
        </div>

        <AnimatePresence>
          {showOrgSelector && userOrganizationsList.length > 1 && (
            <motion.div
            ...
            >
              <p className="text-xs text-muted-foreground mb-3 flex items-center gap-2">
                ...
                Your Organizations ({userOrganizationsList.length})
              </p>
              <div className="space-y-2">
                {userOrganizationsList.map((org: any, idx: number) => (
                  <div
                    key={org.id}
                  ...
                  >
                    <div className="flex-1">
                      ...
                      {org.description && <p className="text-xs text-muted-foreground mt-1">{org.description}</p>}
                      {org.created_at && (
                    ...
                    {organizationId === org.id && <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />}
                  </div>
                ))}
              </div>
            </motion.div>
  ...
  // Superadmin Organization Overview Component
  const SuperadminOrgOverview = () => {
    if (!isSuperadmin || !stats) return null;
    ...
    return (
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <MagneticButton onClick={() => navigate("/organizations")} className="text-xs" variant="outline">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="p-3 rounded-lg bg-white/5 border border-white/10">
            <p className="text-2xl font-bold text-gradient">
              <AnimatedCounter value={stats?.totalOrgs || 0} />
            </p>
          ...
          <div className="p-3 rounded-lg bg-white/5 border border-white/10">
            <p className="text-2xl font-bold text-gradient">
              <AnimatedCounter value={stats?.totalLeads || 0} />
            </p>
  ...
  return (
    <div className="relative min-h-full">
      ...
      <div className="relative z-10 space-y-8">
        ...
        <SuperadminOrgOverview />

        {/* Organization Selector for non-superadmin org users */}
