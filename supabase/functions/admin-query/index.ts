    ...
    switch (action) {
      ...
      case "list_live_classes": {
        const { data } = await supabase
          .from("live_classes")
          .select(
            "*, batches(name, teacher_id, courses(delivery_mode)), schedules(date, start_time, end_time, title, room)",
          )
      ...
      case "list_teachers": {
        ...
        const { data: rolesData } = await supabase.from("user_roles").select("role").eq("user_id", callerId);
        ...
        const { data: roleRows } = await supabase.from("user_roles").select("user_id").eq("role", targetRole);
        ...
        if (!isSuperadmin) {
          ...
          if (wantTeachers) {
            ...
            const { data: orgBatches } = await supabase.from("batches").select("id").eq("organization_id", callerOrgId);
      ...
      case "create_user": {
        ...
        await supabase
          .from("profiles")
          .update({ display_name, organization_id: organization_id || null })
      ...
      case "list_org_subscriptions": {
        const { data } = await supabase
          ...
          .select("*, organizations(name), subscription_plans(name, price, billing_cycle)")
          .order("created_at", { ascending: false });
        result = data ?? [];
        break;
      }
      case "assign_org_subscription": {
        ...
        if (existing) {
          const { error } = await supabase
            .from("org_subscriptions")
            .update({ plan_id, status: "active", starts_at: new Date().toISOString(), expires_at: expires_at || null })
            .eq("organization_id", organization_id);
          if (error) throw error;
        } else {
          const { error } = await supabase
            .from("org_subscriptions")
            .insert({ organization_id, plan_id, expires_at: expires_at || null });
          if (error) throw error;
        }
        result = { success: true };
        break;
      ...
      case "list_activity_logs": {
        ...
        if (userIds.length) {
          ...
          for (const p of profs ?? [])
            profileMap[p.user_id] = { name: p.display_name || p.email || "Unknown", email: p.email || "—" };
        }
      ...
      case "batch_student_count": {
        ...
        const assertBatchInScope = async (batchId: string) => {
          ...
          const { data: b } = await supabase.from("batches").select("organization_id").eq("id", batchId).maybeSingle();
        ...
        if (action === "update_batch") {
          ...
          if (Object.prototype.hasOwnProperty.call(updates, "teacher_id")) {
            const { data: prev } = await supabase.from("batches").select("teacher_id, name").eq("id", id).maybeSingle();
